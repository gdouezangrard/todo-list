...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="/tasks">
        <xsl:for-each select="task">
            <xsl:sort select="substring(@deadline,9,6)" />
            <xsl:sort select="substring(@deadline,4,3)" />
            <xsl:sort select="substring(@deadline,1,0)" />
            <xsl:sort select="@priority" order="descending" />
        <xsl:element name="tr">
            <xsl:if test="'true'=@done">
        ...
        <html>
            ...
            <body>
                ...
                <div data-role="content">
                    <!-- tri par deadline (plus recent en 1er) puis par priorite (de la plus petite a la plus grande) -->
                    <table>
                        <tr>
                            <th>Finie
                            </th>
